...
# 관측소 위치와 발전량 정보, 지리계수 활용해서 지역별 최종 발전량 계산하기 (루틴 4-2)

def regional_sunenergy(extracted_location_forecast,daily_predicted_value_later_date_str):

    if extracted_location_forecast == '강원영동' or extracted_location_forecast == '강원영서':
       regional_daily_predicted_value_later_date_str = daily_predicted_value_later_date_str * 0,738
    elif extracted_location_forecast == '서울.인천.경기':
       regional_daily_predicted_value_later_date_str = daily_predicted_value_later_date_str * 0,368
    elif extracted_location_forecast == '제주도':
       regional_daily_predicted_value_later_date_str = daily_predicted_value_later_date_str * 0,485
    elif extracted_location_forecast == '전라남도':
       regional_daily_predicted_value_later_date_str = daily_predicted_value_later_date_str * 3.0
    elif extracted_location_forecast == '충청남도' or extracted_location_forecast == '충청북도' or  extracted_location_forecast == '전북특별자치도' :
       regional_daily_predicted_value_later_date_str = daily_predicted_value_later_date_str * 1.0
    elif extracted_location_forecast == '경상남도':
       regional_daily_predicted_value_later_date_str = daily_predicted_value_later_date_str * 0.562
    elif extracted_location_forecast == '경상북도':
       regional_daily_predicted_value_later_date_str = daily_predicted_value_later_date_str * 0.932
   
    return regional_daily_predicted_value_later_date_str

regional_daily_predicted_value_later_date_str = regional_sunenergy(extracted_location_forecast,daily_predicted_value_later_date_str)


 
# "later_date_str"의 하늘 상태 가져오기(루틴 4-3)
sky_value_for_later_date_str = get_mode_of_sky(df_forecast_API) 
print(sky_value_for_later_date_str)
#"later_date_str"의 계절 정보  가져오기(루틴 4-4)
season_for_later_date_str= get_season_final(df_forecast_API) 
season_for_later_date_str
print(season_for_later_date_str)
 
# 최종 배터리 충전량 계산(루틴 4-5)
# 태양광발전량, 하늘상태, 계절정보를 종합해 최종충전량 계산하기
combined_battery_charge = decide_final_charge(regional_daily_predicted_value_later_date_str, sky_value_for_later_date_str, season_for_later_date_str)
...
result = { 
    ...
    'region' : [extracted_location_forecast] # 발전소 위치
